...
export default function GSTReports() {
    ...
    const exportGSTR1 = () => {
        if (orders.length === 0) return alert("Bhai, GSTR-1 ke liye is mahine koi sales nahi hai!")

        const headers = ["Invoice Number", "Date", "Customer Name", "Material Type", "Taxable Value", "GST Rate (%)", "GST Amount", "Total Amount"]
        const rows = orders.map(o => [
            `INV-${o.order_number}`,
            new Date(o.order_date).toLocaleDateString(),
            o.customer_name,
            o.material_type === 'CLIENT' ? 'Job Work' : 'Sale',
        ...
        a.href = url
        a.download = `GSTR1_Report_${month}.csv`
        a.click()
    }
    ...
    return (
        <div style={{ maxWidth: '1400px', margin: '0 auto', paddingBottom: '4rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem', marginBottom: '2.5rem' }}>
                ...
                <div style={{ ...cardStyle }}>
                    <div style={cardHeaderStyle}>Filing Status</div>
                    <div style={{ fontSize: '1.1rem', fontWeight: 700, margin: '0.4rem 0' }}>{monthName}</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ fontSize: '0.8rem', color: '#64748b' }}>GSTR-1</span>
                            <span style={statusTagStyle('#f0fdf4', '#16a34a')}>Filed</span>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ fontSize: '0.8rem', color: '#64748b' }}>GSTR-3B</span>
                            <span style={statusTagStyle('#fff7ed', '#ea580c')}>Due 20th Feb</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div style={{ borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 1.5rem' }}>
                    ...
                    <div>
                        <input
                            ...
                            style={{ border: 'none', padding: '0.5rem', fontWeight: 600, color: '#475569', outline: 'none' }}
                        />
                    </div>
                </div>
